...
public class ThirdTest {
    ...
    double twobagtest[] = new double[]{0,3000,1000,4000,450,1450,4450,450,1450,4450,450,1450,4450};
    //标记当前运行的测试用例的下标
    int onebag_index = 0;
    int twobag_index = 0;
    ...
    public void onebag(int type,int bag_num) {
        ...
        bags[3] = new bag(0,0);
        bags[4] = new bag(0,0);
        assertEquals(onebagtest[onebag_index++], Work.get_no_2(type,bag_num,bags));
    }
    ...
    public void twobag(int type,int bag_num,int bag_1_zhong, int bag_1_chicun,int bag_2_zhong,int bag_2_chicun,int bag_3_zhong,int bag_3_chicun){
        ...
        assertEquals(twobagtest[twobag_index++],Work.get_no_2(type,bag_num,bags));
    }

    @BeforeClass
    public static void beforeClass(){
        System.out.println("----正在运行中西亚航线函数测试用例----");
    }

    //在当前测试类结束时运行。
    @AfterClass
    public static void afterClass(){
        System.out.println("----结束测试中西亚航线函数测试----");
    }
}
